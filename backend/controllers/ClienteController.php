<?php
// controllers/ClienteController.php

class ClienteController
{
  private ClienteModel $model;

  public function __construct()
  {
    $this->model = new ClienteModel();
  }

  public function index($tokenData): void
  {
    Response::json(200, $this->model->getAll());
  }

  public function show(int $id, $tokenData): void
  {
    $item = $this->model->getById($id);
    if (!$item) {
      Response::json(404, ['error' => 'Cliente no encontrado']);
    }
    Response::json(200, $item);
  }

  public function store($tokenData): void
  {
    $data = json_decode(file_get_contents('php://input'), true);

    if (empty($data['nombre_empresa'])) {
      Response::json(400, ['error' => 'Campo requerido: nombre_empresa']);
    }

    $id = $this->model->create($data);
    Response::json(201, ['id' => $id, 'mensaje' => 'Cliente creado correctamente']);
  }

  public function update(int $id, $tokenData): void
  {
    $data = json_decode(file_get_contents('php://input'), true);

    if (empty($data)) {
      Response::json(400, ['error' => 'No se enviaron datos para actualizar']);
    }

    $ok = $this->model->update($id, $data);
    if (!$ok) {
      Response::json(404, ['error' => 'Cliente no encontrado o sin cambios']);
    }
    Response::json(200, ['mensaje' => 'Cliente actualizado correctamente']);
  }

  public function destroy(int $id, $tokenData): void
  {
    $this->model->delete($id);
    Response::json(200, ['mensaje' => 'Cliente desactivado correctamente']);
  }
}
